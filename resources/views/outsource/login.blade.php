...
<style>
    ...
    <div class="hrm-login-card mb-0 card shadow-xl border border-slate-200 dark:border-zink-500 relative overflow-hidden">
        ...
        <div class="px-8 py-10 card-body">
            ...
            @endif
            @if(session('feedback') === 'logged_out')
            ...
            @if(session('outsource_login_access'))
                <div class="px-4 py-3 mt-5 text-sm text-custom-500 border border-custom-200 rounded-lg bg-custom-50 dark:bg-zink-600 dark:border-zink-500">
                    Already logged in as {{ session('outsource_company_name') }}.
                    <a href="{{ route('outsource.dashboard') }}" class="font-semibold underline">Open Dashboard</a>
                </div>
            @endif

            <form action="{{ route('outsource.login.submit') }}" class="mt-8" method="POST" autocomplete="on">
                @csrf
