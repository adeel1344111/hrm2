    ...
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center md:justify-between print:hidden">
            <div class="grow">
                ...
                <p class="text-slate-500 dark:text-zink-200">{{ $company }} · {{ $submissions->count() }} records · {{ $totalSales ?? 0 }} sales</p>
            </div>
            <form method="GET" action="{{ route('outsource.report') }}" class="flex flex-wrap gap-2 items-end">
                <div>
                    <label class="inline-block mb-2 text-base font-medium">From</label>
                    <input type="date" name="start_date" value="{{ $start_date }}" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500">
                </div>
                <div>
                    <label class="inline-block mb-2 text-base font-medium">To</label>
                    <input type="date" name="end_date" value="{{ $end_date }}" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500">
                </div>
