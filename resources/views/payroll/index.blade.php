                        ...
                        <div class="flex items-center gap-3">
                            <div class="flex gap-2">
                                <a href="{{ route('payroll.index', ['last_month' => !$lastMonth]) }}" 
                                   class="btn {{ $lastMonth ? 'bg-custom-500 text-white border-custom-500' : 'bg-slate-100 border-slate-200 text-slate-500' }} hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                    <i data-lucide="{{ $lastMonth ? 'calendar' : 'calendar-check' }}" class="w-4 h-4 mr-1"></i>
                                    {{ $lastMonth ? 'Current Month' : 'Last Month' }}
                                </a>
                                <a href="{{ route('payroll.export', ['last_month' => $lastMonth]) }}" 
                    ...
                    @if(count($payrolls) > 0)
                                        ...
                                        <tr class="bg-white border-b border-slate-200 dark:bg-zink-700 dark:border-zink-500 hover:bg-slate-50 dark:hover:bg-zink-600">
                                            <td class="px-6 py-4">
                                                <div class="flex items-center">
                                                    ...
                                                    <div>
                                                        <div class="font-medium text-slate-900 dark:text-zink-100">{{ $payroll['name'] }}</div>
                                                        <div class="text-sm text-slate-500 dark:text-zink-400">{{ $payroll['employee_code'] }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                                            ...
                                            <td class="px-6 py-4">
                                                <span class="font-medium">PKR {{ number_format($payroll['basic_salary'] + $payroll['punctuality']) }}</span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="font-medium">PKR {{ number_format($payroll['bonus']) }}</span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="font-medium">PKR {{ number_format($payroll['arrears_amount'] ?? 0) }}</span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="font-medium">PKR {{ number_format($payroll['training_bonus']) }}</span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="font-medium">PKR {{ number_format($payroll['dock']) }}</span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="font-medium">{{ $payroll['late_count'] }}</span>
                                            </td>
                                            <td class="px-6 py-4">
                                            ...
                                            <td class="px-6 py-4">
                                                <span class="font-medium text-orange-600 dark:text-orange-400">{{ $payroll['unpaid_count'] }}</span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="font-medium text-purple-600 dark:text-purple-400">{{ $payroll['ncns_count'] }}</span>
                                            </td>
                                            <td class="px-6 py-4">
                                            ...
                                            <td class="px-6 py-4">
                                                <span class="font-bold text-lg text-custom-500 dark:text-custom-400">
                                                    PKR {{ number_format($payroll['final_salary']) }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-2">
                                                    <a href="{{ route('payroll.show', $payroll['employee_code']) }}?last_month={{ $lastMonth }}" 
                        ...
                        <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4 mt-8">
                            ...
                            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                                <div class="card-body">
                                    <div class="text-center">
                                        <h6 class="text-3xl font-bold text-blue-600 dark:text-blue-400 mb-2">{{ count($payrolls) }}</h6>
                            ...
                            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                                <div class="card-body">
                                    <div class="text-center">
                                        <h6 class="text-3xl font-bold text-green-600 dark:text-green-400 mb-2">{{ number_format(array_sum(array_column($payrolls, 'final_salary')) / 1000, 0) }}K</h6>
                                        <p class="text-slate-600 dark:text-zink-300 font-medium mb-1">Total Payroll</p>
                                        <p class="text-xs text-slate-500 dark:text-zink-400 mb-3">PKR {{ number_format(array_sum(array_column($payrolls, 'final_salary'))) }}</p>
                            ...
                            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                                <div class="card-body">
                                    <div class="text-center">
                                        <h6 class="text-3xl font-bold text-purple-600 dark:text-purple-400 mb-2">{{ $payrolls[0]['period'] ?? 'N/A' }}</h6>
                            ...
                            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                                <div class="card-body">
                                    <div class="text-center">
                                        <h6 class="text-3xl font-bold text-orange-600 dark:text-orange-400 mb-2">{{ count($payrolls) > 0 ? round(array_sum(array_column($payrolls, 'final_salary')) / count($payrolls)) : 0 }}</h6>
...
<div id="custom-export-modal" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-sm overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4">
        <div class="relative w-full max-w-lg p-6 bg-white rounded-lg shadow-xl dark:bg-zink-700">
            ...
            <form action="{{ route('payroll.custom-export') }}" method="POST">
                @csrf
                <div class="mb-4">
                ...
                <div class="mb-4">
                     <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="last_month" value="1" class="form-checkbox border-slate-200 dark:border-zink-500 text-custom-500 focus:ring-0" {{ $lastMonth ? 'checked' : '' }}>
