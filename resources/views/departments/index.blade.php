    ...
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        ...
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4 mb-6">
            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                <div class="card-body">
                    <div class="text-center">
                        <h6 class="text-3xl font-bold text-blue-600 mb-2">{{ $departments->count() }}</h6>
            ...
            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                <div class="card-body">
                    <div class="text-center">
                        <h6 class="text-3xl font-bold text-green-600 mb-2">{{ $departments->where('status', 'active')->count() }}</h6>
                        ...
                        <div class="flex items-center justify-center text-green-500 text-sm">
                            ...
                            <span>{{ $departments->count() > 0 ? round(($departments->where('status', 'active')->count() / $departments->count()) * 100) : 0 }}%</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                <div class="card-body">
                    <div class="text-center">
                        <h6 class="text-3xl font-bold text-red-600 mb-2">{{ $departments->where('status', 'inactive')->count() }}</h6>
                        ...
                        <div class="flex items-center justify-center text-red-500 text-sm">
                            ...
                            <span>{{ $departments->where('status', 'inactive')->count() > 0 ? 'Needs attention' : 'All good' }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                <div class="card-body">
                    <div class="text-center">
                        <h6 class="text-3xl font-bold text-purple-600 mb-2">{{ \App\Models\User::count() }}</h6>
                    ...
                    <table class="w-full whitespace-nowrap" id="departmentsTable">
                        ...
                        <tbody>
                            @forelse($departments as $department)
                                <tr class="border-b border-slate-200 dark:border-zink-500 hover:bg-slate-50 dark:hover:bg-zink-600 transition-colors" data-department-id="{{ $department->id }}">
                                    <td class="px-3.5 py-2.5">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h6 class="text-15 font-semibold">{{ $department->name }}</h6>
                                                <p class="text-xs text-slate-500 dark:text-zink-200">ID: {{ $department->id }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-3.5 py-2.5">
                                    ...
                                    <td class="px-3.5 py-2.5">
                                        <div class="flex items-center gap-1">
                                            <span class="px-2 py-1 text-xs font-medium text-purple-600 bg-purple-100 rounded-full dark:bg-purple-500/20 dark:text-purple-400">
                                                {{ \App\Models\User::where('department', $department->name)->count() }} users
                                            </span>
                                        </div>
                                    </td>
                                    <td class="px-3.5 py-2.5">
                                        <div class="text-sm text-slate-500 dark:text-zink-200">
                                            ...
                                            <br>
                                            <span class="text-xs">{{ $department->created_at->diffForHumans() }}</span>
                                        </div>
                                    </td>
                                    <td class="px-3.5 py-2.5">
                                        <div class="flex items-center gap-2">
                                            <button type="button" class="text-blue-500 hover:text-blue-600 p-1 rounded hover:bg-blue-50 dark:hover:bg-blue-500/20" onclick="viewDepartment({{ $department->id }})" title="View Details">
                                                <i data-lucide="eye" class="w-4 h-4"></i>
                                            </button>
                                            <a href="{{ route('departments.edit', $department) }}" class="text-green-500 hover:text-green-600 p-1 rounded hover:bg-green-50 dark:hover:bg-green-500/20" title="Edit">
                                                <i data-lucide="edit" class="w-4 h-4"></i>
                                            </a>
                                            <button type="button" class="text-orange-500 hover:text-orange-600 p-1 rounded hover:bg-orange-50 dark:hover:bg-orange-500/20" onclick="toggleStatus({{ $department->id }}, '{{ $department->status }}')" title="Toggle Status">
                                                <i data-lucide="{{ $department->status === 'active' ? 'pause' : 'play' }}" class="w-4 h-4"></i>
                                            </button>
                                            <form action="{{ route('departments.destroy', $department) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this department? This action cannot be undone.')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="7" class="px-3.5 py-12 text-center text-slate-500 dark:text-zink-200">
                                        <div class="flex flex-col items-center gap-4">
                                            ...
                                            <a href="{{ route('departments.create') }}" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600">
