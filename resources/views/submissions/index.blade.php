    ...
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        ...
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4 mb-6">
            ...
            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                <div class="card-body">
                    <div class="text-center">
                        <h6 class="text-3xl font-bold text-green-600 dark:text-green-400 mb-2">{{ \Carbon\Carbon::parse($stats['date'])->format('d') }}</h6>
                        <p class="text-slate-600 dark:text-zink-300 font-medium mb-1">Selected Date</p>
                        <p class="text-xs text-slate-500 dark:text-zink-400 mb-3">{{ \Carbon\Carbon::parse($stats['date'])->format('M Y') }}</p>
                        <div class="flex items-center justify-center text-green-500 dark:text-green-400 text-sm">
                            <i data-lucide="calendar" class="w-4 h-4 mr-1"></i>
                            <span>{{ \Carbon\Carbon::parse($stats['date'])->format('D') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                <div class="card-body">
                    <div class="text-center">
                        <h6 class="text-3xl font-bold text-purple-600 dark:text-purple-400 mb-2">{{ $stats['total'] > 0 ? round($stats['total'] / 8) : 0 }}</h6>
            ...
            <div class="card hover:shadow-lg transition-all duration-300 cursor-pointer group">
                <div class="card-body">
                    <div class="text-center">
                        <h6 class="text-3xl font-bold text-orange-600 dark:text-orange-400 mb-2">{{ $stats['total'] > 0 ? '100' : '0' }}%</h6>
                    ...
                    <table id="submissionsTable" class="w-full text-left">
                        ...
                        <tbody class="bg-white dark:bg-zink-700 divide-y divide-slate-200 dark:divide-zink-500">
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-zink-600">
                                ...
                                @if(auth()->user()->isVerificationOfficer() || auth()->user()->isAdmin() || auth()->user()->isFloorManager() || auth()->user()->isTeamLead())
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900 dark:text-zink-100">
                                    @if($submission->submission_type === 'verification' && isset($submission->did))
                                ...
                                @if(auth()->user()->user_type !== 'agent')
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-zink-400">
                                    @if(isset($submission->submission_type) && $submission->submission_type === 'csr')
                                        @if($submission->is_sale)
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        @if(auth()->user()->isAdmin())
                                        <a href="{{ route('submissions.edit', ['submission' => $submission->id, 'type' => $submission->submission_type]) }}" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300" title="Edit">
                                            <i data-lucide="edit" class="w-4 h-4"></i>
                                        </a>
                                        
                                        <form action="{{ route('submissions.destroy', ['submission' => $submission->id, 'type' => $submission->submission_type]) }}" method="POST" class="inline-block m-0 p-0" onsubmit="return confirm('Are you sure you want to delete this submission?')">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <a href="{{ route('submissions.show', ['submission' => $submission->id, 'type' => $submission->submission_type]) }}" class="text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-300" title="View">
                            ...
                            @empty
                            <tr>
                                <td colspan="{{ auth()->user()->user_type === 'agent' ? '6' : (auth()->user()->isVerificationOfficer() || auth()->user()->isAdmin() || auth()->user()->isFloorManager() || auth()->user()->isTeamLead() ? '8' : '7') }}" class="px-6 py-4 text-center text-slate-500 dark:text-zink-400">
