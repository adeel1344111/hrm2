...
@section('title') Designation Details @endsection
@section('content')
<!-- Page-content -->
<div class="group-data-[sidebar-size=lg]:ltr:md:ml-vertical-menu group-data-[sidebar-size=lg]:rtl:md:mr-vertical-menu group-data-[sidebar-size=md]:ltr:ml-vertical-menu-md group-data-[sidebar-size=md]:rtl:mr-vertical-menu-md group-data-[sidebar-size=sm]:ltr:ml-vertical-menu-sm group-data-[sidebar-size=sm]:rtl:mr-vertical-menu-sm pt-[calc(theme('spacing.header')_*_1)] pb-[calc(theme('spacing.header')_*_0.8)] px-4 group-data-[navbar=bordered]:pt-[calc(theme('spacing.header')_*_1.3)] group-data-[navbar=hidden]:pt-0 group-data-[layout=horizontal]:mx-auto group-data-[layout=horizontal]:max-w-screen-2xl group-data-[layout=horizontal]:px-0 group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:ltr:md:ml-auto group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:rtl:md:mr-auto group-data-[layout=horizontal]:md:pt-[calc(theme('spacing.header')_*_1.6)] group-data-[layout=horizontal]:px-3 group-data-[layout=horizontal]:group-data-[navbar=hidden]:pt-[calc(theme('spacing.header')_*_0.9)]">
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
            <div class="grow">
                <h5 class="text-16">Designation Details</h5>
            </div>
            <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
                ...
                <li class="text-slate-700 dark:text-zink-100">{{ $designation->name }}</li>
            </ul>
        </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="flex items-center gap-3 mb-4">
                            <div class="flex items-center justify-center w-12 h-12 bg-purple-100 dark:bg-purple-900/20 rounded-lg">
                                <i data-lucide="briefcase" class="w-6 h-6 text-purple-600 dark:text-purple-400"></i>
                            </div>
                            <div>
                                <h6 class="text-15">{{ $designation->name }}</h6>
                                <p class="text-slate-500 dark:text-zink-200">Designation Information</p>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <label class="block text-sm font-medium text-slate-500 dark:text-zink-400 mb-1">Department</label>
                                <div class="flex items-center p-3 bg-slate-50 dark:bg-zink-700 rounded-lg">
                                    <span class="text-slate-900 dark:text-zink-100 font-medium">{{ $designation->department }}</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center p-3 bg-slate-50 dark:bg-zink-700 rounded-lg">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                        @if($designation->status === 'active') bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300
                        ...
                        <div class="mt-4">
                            ...
                            <div class="p-3 bg-slate-50 dark:bg-zink-700 rounded-lg">
                                <p class="text-slate-900 dark:text-zink-100">{{ $designation->description }}</p>
                            </div>
                        </div>
                        @endif
                    </div>
