{{-- Outsource partner sidebar — same visual language as main HRM sidebar --}}
<div>
            ...
            <div class="flex flex-col items-center text-center">
                <div class="mb-3">
                    <div class="flex items-center justify-center font-medium rounded-full size-20 shrink-0 bg-slate-200 text-slate-800 dark:text-zink-50 dark:bg-zink-600 text-2xl">
                        {{ strtoupper(substr(session('outsource_company_name', 'O'), 0, 1)) }}
                    </div>
                </div>
                <h3 class="text-slate-900 dark:text-zink-100 font-bold text-sm mb-1">Outsource</h3>
                <p class="text-slate-600 dark:text-zink-300 text-xs font-medium mb-1">{{ session('outsource_company_name') }}</p>
                <div>
                    <span class="text-slate-500 dark:text-zink-400 text-xs font-medium">{{ session('outsource_admin_name') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <ul class="group-data-[layout=horizontal]:flex group-data-[layout=horizontal]:flex-col group-data-[layout=horizontal]:md:flex-row" id="navbar-nav">
        ...
        <li class="relative group/sm mt-2">
            ...
            <form id="outsource-logout-form" action="{{ route('outsource.logout') }}" method="POST" class="hidden">@csrf</form>
        </li>
    </ul>
</div>
