...
<body class="text-base bg-body-bg text-body font-public dark:text-zink-100 dark:bg-zink-800 group-data-[skin=bordered]:bg-body-bordered group-data-[skin=bordered]:dark:bg-zink-700">
                        ...
                        <div class="flex gap-3 ms-auto">
                                ...
                                <div class="absolute z-50 hidden ltr:text-left rtl:text-right bg-white rounded-md shadow-md !top-4 dropdown-menu min-w-[20rem] lg:min-w-[26rem] dark:bg-zink-600" aria-labelledby="notificationDropdown">
                                    <div class="p-4">
                                        <div class="flex items-center justify-between mb-4">
                                            <h6 class="text-16">Notifications 
                                                @if($unreadCount > 0)
                                                <span class="inline-flex items-center justify-center w-5 h-5 ml-1 text-[11px] font-medium border rounded-full text-white bg-orange-500 border-orange-500" id="unread-count">{{ $unreadCount }}</span>
                                                @endif
                                            </h6>
                                            @if($unreadCount > 0)
                                    ...
                                    <div data-simplebar="" class="max-h-[350px]">
                                        <div class="flex flex-col gap-1" id="notification-list">
                                            ...
                                            <a href="javascript:void(0);" onclick="markAsRead({{ $notification->id }})" class="flex gap-3 p-4 product-item hover:bg-slate-50 dark:hover:bg-zink-500 {{ $notification->isUnread() ? 'bg-blue-50 dark:bg-zink-700' : '' }}">
                                                <div class="flex items-center justify-center w-10 h-10 rounded-md shrink-0 
                                                    @if($notification->color == 'blue') bg-blue-100 @endif
                                                    @if($notification->color == 'green') bg-green-100 @endif
                                                    @if($notification->color == 'red') bg-red-100 @endif
                                                    @if($notification->color == 'yellow') bg-yellow-100 @endif
                                                    @if($notification->color == 'purple') bg-purple-100 @endif
                                                ">
                                                    <i data-lucide="{{ $notification->icon }}" class="w-5 h-5 
                                                        @if($notification->color == 'blue') text-blue-500 @endif
                                                        @if($notification->color == 'green') text-green-500 @endif
                                                        @if($notification->color == 'red') text-red-500 @endif
                                                        @if($notification->color == 'yellow') text-yellow-500 @endif
                                                        @if($notification->color == 'purple') text-purple-500 @endif
                                                    "></i>
                                                </div>
                                                <div class="grow">
                                                    <h6 class="mb-1 font-medium">{{ $notification->title }}</h6>
                                                    <p class="mb-0 text-sm text-slate-500 dark:text-zink-300">{{ $notification->message }}</p>
                                                    <p class="mb-0 text-xs text-slate-400 dark:text-zink-400 mt-1">
                                                        <i data-lucide="clock" class="inline-block w-3 h-3 mr-1"></i>
                                                        <span class="align-middle">{{ $notification->created_at->diffForHumans() }}</span>
                                                    </p>
                                                </div>
                            ...
                            <div class="relative flex items-center dropdown h-header">
                                <button type="button" class="inline-block p-0 transition-all duration-200 ease-linear bg-topbar rounded-full text-topbar-item dropdown-toggle btn hover:bg-topbar-item-bg-hover hover:text-topbar-item-hover group-data-[topbar=dark]:text-topbar-item-dark group-data-[topbar=dark]:bg-topbar-dark group-data-[topbar=dark]:hover:bg-topbar-item-bg-hover-dark group-data-[topbar=dark]:hover:text-topbar-item-hover-dark group-data-[topbar=brand]:bg-topbar-brand group-data-[topbar=brand]:hover:bg-topbar-item-bg-hover-brand group-data-[topbar=brand]:hover:text-topbar-item-hover-brand group-data-[topbar=dark]:dark:bg-zink-700 group-data-[topbar=dark]:dark:hover:bg-zink-600 group-data-[topbar=brand]:text-topbar-item-brand group-data-[topbar=dark]:dark:hover:text-zink-50 group-data-[topbar=dark]:dark:text-zink-200" id="dropdownMenuButton" data-bs-toggle="dropdown">
                                    <div class="bg-pink-100 rounded-full">
                                        @if(auth()->user()->profile_picture)
                                            <img src="{{ asset('assets/images/' . auth()->user()->profile_picture) }}" alt="" class="w-[37.5px] h-[37.5px] rounded-full object-cover">
                                        @else  
                                ...
                                <div class="absolute z-50 hidden p-4 ltr:text-left rtl:text-right bg-white rounded-md shadow-md !top-4 dropdown-menu min-w-[14rem] dark:bg-zink-600" aria-labelledby="dropdownMenuButton">
                                    ...
                                    <ul>
                                        <li>
                                            <a class="block ltr:pr-4 rtl:pl-4 py-1.5 text-base font-medium transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:text-custom-500 focus:text-custom-500 dark:text-zink-200 dark:hover:text-custom-500 dark:focus:text-custom-500" href="{{ route('settings.index') }}">
                                                <i data-lucide="user-2" class="inline-block size-4 ltr:mr-2 rtl:ml-2"></i> Profile
                                            </a>
                                        </li>
                                        <li class="pt-2 mt-2 border-t border-slate-200 dark:border-zink-500">
                                            <a class="block ltr:pr-4 rtl:pl-4 py-1.5 text-base font-medium transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:text-custom-500 focus:text-custom-500 dark:text-zink-200 dark:hover:text-custom-500 dark:focus:text-custom-500" href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
    ...
    <script src="{{ URL::to('assets/js/hrm-system.bundle.js') }}"></script>
