            ...
            <div class="card-body">
                ...
                <div class="flex flex-col gap-3 mb-4 md:flex-row md:items-center md:justify-between print:hidden">
                    <div class="text-sm text-slate-500 dark:text-zink-200">
                        <span id="agentCount">{{ $agents->count() }}</span> active agent(s)
                    </div>
                    ...
                    <table class="w-full" id="agentsInfoTable">
                        ...
                        <tbody class="bg-white divide-y divide-slate-200 dark:bg-zink-700 dark:divide-zink-500">
                            ...
                            <tr id="row-{{ $agent->id }}"
                                class="agent-row"
                                data-id="{{ $agent->id }}"
                                data-designation="{{ $agent->designation ?? '' }}"
                                ...
                                <td class="px-2 py-2">
                                    <input type="text" name="contact_number" value="{{ $agent->contact_number }}"
                                        class="form-input text-xs min-w-[8rem] border-slate-200 dark:border-zink-500 dark:bg-zink-600 dark:text-zink-100">
                                </td>
                                <td class="px-2 py-2">
                                    <input type="text" name="emergency_contact" value="{{ $agent->emergency_contact }}"
                                        class="form-input text-xs min-w-[8rem] border-slate-200 dark:border-zink-500 dark:bg-zink-600 dark:text-zink-100">
                                </td>
                                <td class="px-2 py-2">
                                    <input type="text" name="cnic" value="{{ $agent->cnic }}"
                                ...
                                <td class="px-2 py-2">
                                    <select name="designation" class="form-input text-xs min-w-[8rem] border-slate-200 dark:border-zink-500 dark:bg-zink-600 dark:text-zink-100">
                                        ...
                                        @foreach($designations as $designation)
                                            <option value="{{ $designation->name }}" {{ $agent->designation === $designation->name ? 'selected' : '' }}>
                                                {{ $designation->name }}
                                            </option>
                                        @endforeach
                                        @if($agent->designation && !$designations->contains('name', $agent->designation))
                                            <option value="{{ $agent->designation }}" selected>{{ $agent->designation }}</option>
                                        @endif
                                    </select>
                                </td>
                                <td class="px-2 py-2">
                                    <input type="date" name="appointment_date" value="{{ $agent->appointment_date ? \Carbon\Carbon::parse($agent->appointment_date)->format('Y-m-d') : '' }}"
                                        class="form-input text-xs min-w-[9rem] border-slate-200 dark:border-zink-500 dark:bg-zink-600 dark:text-zink-100">
                                </td>
                                <td class="px-2 py-2">
                                ...
                                <td class="px-2 py-2">
                                    <select name="floor_manager_id" class="form-input text-xs min-w-[9rem] border-slate-200 dark:border-zink-500 dark:bg-zink-600 dark:text-zink-100">
                                        ...
                                        @foreach($floorManagers as $fm)
                                            <option value="{{ $fm->id }}" {{ (int)$agent->floor_manager_id === (int)$fm->id ? 'selected' : '' }}>
                                                {{ $fm->name }}{{ $fm->employee_id ? ' ('.$fm->employee_id.')' : '' }}
                                            </option>
                                        @endforeach
                                    </select>
                                </td>
                                <td class="px-2 py-2">
                                    <select name="team_lead_id" class="form-input text-xs min-w-[9rem] border-slate-200 dark:border-zink-500 dark:bg-zink-600 dark:text-zink-100">
                                        ...
                                        @foreach($teamLeads as $tl)
                                            <option value="{{ $tl->id }}" {{ (int)$agent->team_lead_id === (int)$tl->id ? 'selected' : '' }}>
                                                {{ $tl->name }}{{ $tl->employee_id ? ' ('.$tl->employee_id.')' : '' }}
                                            </option>
                                        @endforeach
                                    </select>
                                </td>
